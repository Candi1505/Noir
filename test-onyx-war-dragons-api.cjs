...
let invocation = null;
sandbox.OnyxCommandCore = { getCurrentUserId: () => "owner-user" };
...
(async () => {
  ...
  assert.equal(profile.public_profile.level, 999);
  assert.equal(sandbox.OnyxWarDragonsAPI.getProfile().public_profile.level, 999);
